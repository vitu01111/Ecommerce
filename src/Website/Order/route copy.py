...
# @order_router.route('/historyUser/<int:user_id>',methods=['GET'])
# def history(user_id):
#     orders = db.session.query(Order).filter_by(user_id=user_id).all()
    
#     return render_template('web/product/historyUser.html', orders=orders)
...
def history(user_id):
    ...
    if q:
        ...
        orders = db.session.query(Order).filter_by(user_id=user_id).all()
    return render_template('web/product/historyUser.html', orders=orders, user_id=user_id)
