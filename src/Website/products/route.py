from flask import Blueprint,current_app, app, render_template, request, flash, redirect, url_for, session
...
@product_bp.route('/confirm/<int:order_id>',methods=['GET','POST'])
def confirmation(order_id):
    #  order=db.session.query(Order).filter_by(id=order_id).first()
    #  orderPrice=db.session.query(Order).filter_by(id=order_id).first()
    #  orderTotal=db.session.query(OrderDetail).filter_by(id=order_id).first()
     return render_template('web/order-confirmation.html',orderID=order_id,aaaa="afs######9999999falfjaf")
